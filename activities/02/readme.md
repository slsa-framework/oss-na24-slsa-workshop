...
The publish attestation hides away details of the development cycle from your consumers. For example, most of them need not know or keep track of which repository your artifact was built from. All that matters to them is the level of integrity protection (SLSA level) the artifact has or whether it was scanned or some other property. If certain consumers want to know more details about your artifacts (e.g., what builder you used, what repository it was built from), they can verify upstream attestations (e.g., SLSA provenance from the builder) themselves using attestations referenced in the publish attestation. Note that if a project is not open source of if it is not built on a public builder (GitHub, GCB), the publish attestation also allows producers to convey certain characteritics about their artifact.
